...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function animateOldStep(stepEl, direction = "forward") {
    ...
    gsap.to(parts, { opacity: 0, y: toY, duration: 0.4, stagger: 0.06, ease: "power2.out", overwrite: true });
  }
  ...
  function scheduleLoopRestart() {
    ...
    if (window.gsap) {
      fillTween = gsap.delayedCall(holdLastStepSeconds, () => {
        updateLineByProgress(0);
        setActiveStep(0, { direction: "backward" });
        playCurrentSegment();
      });
    } else {
      fillTween = setTimeout(() => {
        updateLineByProgress(0);
        setActiveStep(0, { direction: "backward" });
        playCurrentSegment();
  ...
  function jumpToStep(index) {
    ...
    if (loop && currentIndex >= stepCount - 1) {
      scheduleLoopRestart();
    } else {
      playCurrentSegment();
    }
