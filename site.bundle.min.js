...
function initSwiperTeam() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!activeEl) return;
    if (!swiperEl.contains(activeEl) && activeEl !== swiperEl) return;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateProjectDetails(swiperInstance) {
    const activeSlide = swiperInstance.slides[swiperInstance.activeIndex];
    if (!activeSlide) return;

    const targets = document.querySelectorAll("[data-project-target]");
    targets.forEach((target) => {
      const key = target.getAttribute("data-project-target");
      const source = activeSlide.querySelector('[data-project-field="' + key + '"]');
  ...
  document.querySelectorAll(".swiper-projects .swiper-slide").forEach((slide, i) => {
    slide.setAttribute("tabindex", "0");
    slide.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  swiperEl.addEventListener("keydown", function (e) {
    const activeInsideSwiper = swiperEl.contains(document.activeElement);
    if (!activeInsideSwiper) return;
