...
window.Webflow.push(function () {
  ...
  function init() {
    const items = Array.from(document.querySelectorAll(".product_item"));
    ...
    items.forEach(function (item) {
      ...
      if (!img || !paragraph || !wrapper) return;
    ...
    function updateActiveItem() {
      const activeItem = hoveredItem || getClosestToViewportCenter();

      itemData.forEach(({ item, tl }) => {
        if (item === activeItem) {
          tl.play();
        } else {
          tl.reverse();
...
// Process Script
document.addEventListener("DOMContentLoaded", function () {
  const dots     = Array.from(document.querySelectorAll(".step_ellipse[data-step]"));
  const steps    = Array.from(document.querySelectorAll(".process_top[data-step]"));
  const labels   = Array.from(document.querySelectorAll(".process_steps .step_text_wrap[data-step]"));
  const lineFill = document.querySelector(".process_line-fill");
  ...
  const countdownStepIndex = 3; // step 4 (0-based index)
  const holdLastStepSeconds = secondsPerStep;

  const stepCount = steps.length;
  let currentIndex = 0;
  let fillTween = null;

  function initStepLabelCharacterStagger() {
    const offsetIncrement = 0.01;
    ...
    labels.forEach(label => {
      ...
      wrapper.setAttribute("data-step-animate-chars", "");
  ...
  initStepLabelCharacterStagger();

  function animateStepContent(stepEl, direction = "forward") {
    if (!stepEl || !window.gsap) return;

    const parts = stepEl.querySelectorAll(".subheading, .display_wrapper, .process_text_wrap, .process_img_wrap");
    gsap.killTweensOf(parts);

    const fromY = direction === "forward" ? 8 : -8;
  ...
  function animateOldStep(stepEl, direction = "forward") {
    if (!stepEl || !window.gsap) return;

    const parts = stepEl.querySelectorAll(".subheading, .display_wrapper, .process_text_wrap, .process_img_wrap");
    gsap.killTweensOf(parts);

    const toY = direction === "forward" ? -8 : 8;
  ...
  function resetLineVisual() {
    if (!lineFill) return;
    lineFill.style.clipPath = "inset(0 0% 0 0)";
  }

  function updateLineByProgress(progress01) {
    if (!lineFill) return;

    const clamped = Math.min(Math.max(progress01, 0), 1);
    lineFill.style.width = clamped * 100 + "%";
    lineFill.style.clipPath = "inset(0 0% 0 0)";
  }

  function updateDotsAndLabels(activeIndex) {
    dots.forEach((dot, i) => dot.classList.toggle("is--active", i <= activeIndex));
    labels.forEach((label, i) => label.classList.toggle("is--active", i <= activeIndex));
  }

  function setActiveStep(newIndex, { direction = "forward" } = {}) {
    newIndex = Math.max(0, Math.min(newIndex, stepCount - 1));
    if (newIndex === currentIndex) return;

    const oldStep = steps[currentIndex];
    if (oldStep) animateOldStep(oldStep, direction);

    steps.forEach((s, i) => s.classList.toggle("is--active", i === newIndex));
    updateDotsAndLabels(newIndex);

    const newStep = steps[newIndex];
    if (newStep) animateStepContent(newStep, direction);

    currentIndex = newIndex;
  }

  function killFillTween() {
    if (!fillTween) return;

    if (window.gsap && fillTween.kill) {
      fillTween.kill();
    } else {
      cancelAnimationFrame(fillTween);
    }

    fillTween = null;
  }

  function playCountdownToRestart() {
    killFillTween();

    if (!lineFill) {
      setActiveStep(0, { direction: "backward" });
      updateLineByProgress(0);
      playCurrentSegment();
      return;
    }
    ...
    if (window.gsap) {
      fillTween = gsap.to(lineFill, {
        clipPath: "inset(0 0% 0 100%)",
        duration: holdLastStepSeconds,
        ease: "none",
        onComplete: () => {
          resetLineVisual();
          updateLineByProgress(0);
      ...
      const durationMs = holdLastStepSeconds * 1000;

      function tick(now) {
        const t = Math.min((now - startTime) / durationMs, 1);
        const leftInset = t * 100;
        lineFill.style.clipPath = `inset(0 0% 0 ${leftInset}%)`;

        if (t < 1) {
          fillTween = requestAnimationFrame(tick);
        } else {
          resetLineVisual();
          updateLineByProgress(0);
          setActiveStep(0, { direction: "backward" });
  ...
  function playCurrentSegment() {
    ...
    const start = stepCount === 1 ? 1 : currentIndex / (stepCount - 1);
    const end   = stepCount === 1 ? 1 : (currentIndex + 1) / (stepCount - 1);

    if (currentIndex >= stepCount - 1) {
      if (loop) {
        if (currentIndex === countdownStepIndex) {
          playCountdownToRestart();
        } else {
          setTimeout(() => {
            updateLineByProgress(0);
            setActiveStep(0, { direction: "backward" });
            playCurrentSegment();
          }, holdLastStepSeconds * 1000);
        }
      } else {
        updateLineByProgress(1);
      }
      return;
    }

    updateLineByProgress(start);
    ...
    if (window.gsap) {
      ...
      fillTween = gsap.to(obj, {
        ...
        ease: "none",
        onUpdate: () => updateLineByProgress(obj.p),
        onComplete: () => {
          setActiveStep(currentIndex + 1, { direction: "forward" });
          playCurrentSegment();
        }
      });
    } else {
      const startTime = performance.now();
      const durationMs = secondsPerStep * 1000;

      function tick(now) {
        const t = Math.min((now - startTime) / durationMs, 1);
        const p = start + (end - start) * t;
        updateLineByProgress(p);
  ...
  function jumpToStep(index) {
    ...
    const start = stepCount === 1 ? 1 : clamped / (stepCount - 1);
    updateLineByProgress(start);

    steps.forEach((s, i) => s.classList.toggle("is--active", i === clamped));
    updateDotsAndLabels(clamped);
    animateStepContent(steps[clamped], direction);

    currentIndex = clamped;

    if (loop && currentIndex >= stepCount - 1 && currentIndex === countdownStepIndex) {
      playCountdownToRestart();
    } else {
      playCurrentSegment();
    }
  ...
  dots.forEach(dot => {
    ...
    dot.addEventListener("click", () => jumpToStep(+dot.dataset.step || 0));
  });

  labels.forEach(label => {
    label.style.cursor = "pointer";
    label.addEventListener("click", () => jumpToStep(+label.dataset.step || 0));
  });

  if (pauseOnHover) {
    const wrapper = document.querySelector(".section_process-scroll") || document.body;

    wrapper.addEventListener("mouseenter", () => {
      if (fillTween && window.gsap && fillTween.pause) fillTween.pause();
    });

    wrapper.addEventListener("mouseleave", () => {
      if (fillTween && window.gsap && fillTween.resume) fillTween.resume();
    });
  }

  steps.forEach((step, i) => step.classList.toggle("is--active", i === 0));
  updateDotsAndLabels(0);
  updateLineByProgress(0);
  animateStepContent(steps[0], "forward");

  if (autoStart) playCurrentSegment();
});
